...
int saveFlightToFile(const Flight* pF, FILE* fp)
{
	...
	data[0] = pF->hour | pF->date.day << 5;
	data[1] = (pF->date.day >> 3) | (pF->date.month << 2) | (pF->date.year << 6);
	data[2] = pF->date.year >> 2;
	data[3] = pF->date.year >> 10;
...
{
	if (fread(pF->originCode, sizeof(char), CODE_LENGTH, fp) != CODE_LENGTH)
	{
	...
	if (fread(pF->destCode, sizeof(char), CODE_LENGTH, fp) != CODE_LENGTH)
	{
		printf("Error read flight\n");
		return 0;
	}
	...
	pF->date.day = ((data[0] & 0xE0) >> 5) | ((data[1] & 0x3) << 3);
	pF->date.month = (data[1] & 0x3C) >> 2;
	pF->date.year = ((data[1] & 0xC0) >> 6) | (data[2] << 2) | (data[3] << 10);
