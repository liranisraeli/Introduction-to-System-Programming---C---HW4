...
int		saveManagerToFile(const AirportManager* pManager, const char* fileName)
{
	...
	while (pNode)
	{
		...
		CHECK_0_MSG_CLOSE_FILE(saveAirportToFile(pNode->key, fp), "Error write airport\n", fp);
...
int		loadManagerFromFile(AirportManager* pManager, const char* fileName)
{
	...
	for (int i = 0; i < pManager->count; i++)
	{
		...
		CHECK_0_MSG_CLOSE_FILE(loadAirportFromFile(pPort, fp), "Error loading airport from file\n", fp);
