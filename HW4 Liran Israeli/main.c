...
{
	if (argc != 3)				//	not enough or too many args
		return EXIT;

	char* authority_filename = argv[1];
	...
	initManagerAndCompany(&manager, &company, authority_filename, company_filename);
	...
	do
	{
		...
		case ePrintCompany:
			printCompany(&company, "Hachi", "Babit", "Ba_Olam", NULL);
			break;
	...
	saveManagerToFile(&manager, authority_filename);
...
int initManagerAndCompany(AirportManager* pManager, Company* pCompany, char* authority_filename, char* company_filename)
{
	int res = initManager(pManager, authority_filename);
	if (!res)
	{
